...
async def cmd_agnt_week_overall(message: Message, state: FSMContext) -> None:
    ...
    async with ChatActionSender.typing(bot=bot, chat_id=message.chat.id):
        await message.answer("🤖 Анализирую историю чатов за последнюю неделю... Это может занять некоторое время.")
...
Отвечай на русском языке, структурированно и подробно."""
            ...
            logger.info("Отправляю запрос в LLM для анализа истории за неделю: %d чатов, %d сообщений", 
            ...
            # Разбиваем результат на части, если он слишком длинный для Telegram
            MAX_MESSAGE_LENGTH = 4000  # Telegram ограничение ~4096 символов
            ...
            if len(analysis_result) <= MAX_MESSAGE_LENGTH:
                ...
                await message.answer(header + analysis_result, parse_mode="HTML")
            else:
                # Разбиваем на части
                header = (
                    f"📊 <b>Анализ истории чатов за неделю</b>\n\n"
                ...
                # Разбиваем анализ на части по разделам
                parts = analysis_result.split("\n\n")
                current_part = ""
                
                for part in parts:
                    if len(current_part) + len(part) + 10 > MAX_MESSAGE_LENGTH:
                        if current_part:
